...
// Telemetry & Error Capture Engine
...
// ------------------------------------------------------------------
// 1. BOOTSTRAP AUTOMATED FLEET TELEMETRY
// ------------------------------------------------------------------
initGlobalTelemetry();

// ------------------------------------------------------------------
// 2. CORE ENGINE (Strict Offline-First & Auth Refresh Handling)
// ------------------------------------------------------------------
    ...
    mutations: {
      ...
      retry: (failureCount, error: any) => {
        ...
        if (isAuthError) {
          ...
          return true; // Re-attempt payload with updated credentials
        }
...
// ------------------------------------------------------------------
// 3. ASYNC STORAGE ADAPTER (IndexedDB)
// ------------------------------------------------------------------
const idbStorage = {
...
// ------------------------------------------------------------------
// 4. EXPLICIT CACHE DEHYDRATION (With iOS RAM Protection)
// ------------------------------------------------------------------
const isIOS =
...
if (!isIOS) {
  ...
  console.warn('[Sync Engine] iOS device detected. Running in memory-buffered RAM mode to prevent WebKit cache purges.');
}

// ------------------------------------------------------------------
// 5. OFFLINE RECOVERY HANDLERS
// ------------------------------------------------------------------
queryClient.resumePausedMutations();

onlineManager.subscribe((isOnline) => {
  if (isOnline) {
    console.log('[Sync Engine] Network restored. Firing queued offline mutations.');
    queryClient.resumePausedMutations();
  }
});

// ------------------------------------------------------------------
// 6. ROUTER & GLOBAL ERROR BOUNDARIES
// ------------------------------------------------------------------
