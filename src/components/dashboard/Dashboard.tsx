...
export default function Dashboard() {
    ...
    cols.push(
      ...
      columnHelper.accessor('today_weight', {
        ...
        cell: (info) => {
          ...
          if (!w) return <span className="text-slate-300 text-[10px] md:text-[11px] lg:text-xs font-medium py-0.5 block w-full text-center">-</span>;
          return (
            <div className="flex items-center justify-center gap-1 lg:gap-1.5 py-0.5 w-full">
              ...
              <span className="text-[10px] md:text-[11px] lg:text-[13px] font-black text-slate-700">{formatWeightDisplay(w.weight_grams, 'g')}</span>
            </div>
          );
        }
      }),
      ...
      columnHelper.accessor('today_feed', {
        ...
        cell: (info) => {
          ...
          if (feeds.length === 0) return <span className="text-slate-300 text-[10px] lg:text-xs font-medium py-0.5 block w-full text-center">-</span>;
      ...
      columnHelper.accessor('last_feed', {
        ...
        cell: (info) => {
          ...
          if (!lastMeal) return <span className="text-slate-300 text-[10px] lg:text-xs font-medium py-0.5 block w-full text-center">No History</span>;
      ...
      columnHelper.accessor('next_feed', {
        ...
        cell: (info) => {
          ...
          if (!nextFeed) return <span className="text-slate-300 text-[10px] lg:text-xs font-medium py-0.5 block w-full text-center">-</span>;
  ...
  // --- REDUCED MINMAX WIDTHS FOR TIGHTER FIT ---
  const tableGridCols = table.getVisibleLeafColumns().map(c => {
    if (c.id === 'name') return screen.isMobile ? 'minmax(90px, 1.2fr)' : screen.isTablet ? 'minmax(110px, 1.2fr)' : 'minmax(140px, 1.5fr)';
    if (c.id === 'species') return screen.isMobile || screen.isTablet ? 'minmax(70px, 0.8fr)' : 'minmax(90px, 1fr)';
    if (c.id === 'location') return screen.isMobile || screen.isTablet ? 'minmax(60px, 0.8fr)' : 'minmax(90px, 1fr)';
    if (c.id === 'today_weight') return screen.isMobile || screen.isTablet ? '45px' : '80px';
                ...
                <React.Fragment key={headerGroup.id}>
                  {headerGroup.headers.map(header => {
                    ...
                    return (
                      <div 
                        key={header.id} 
                        // --- MINIMAL PADDING FOR NARROW SCREENS ---
